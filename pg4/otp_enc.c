...
int main(int argc, char *argv[])
{
    ...
    int check = 0;
    int checkSize = 0;
    char serverCheck[] = "@@PLAIN TEXT%%KEY TEXT**";
    ...
    if(strcmp(buffer, "@@PLAIN TEXT%%KEY TEXT**") != 0){
    ...
    if (check == 1 && checkSize == 1){ //once both checks return correct our variables are set to 1 and the rest of the code can execute
     ...
     //This is the same code given in the lecture it reads in 100 bytes at a time and then concatenate to buffer
     memset(buffer, '\0', sizeof(buffer));// intializing buffer array
     memset(completeMessage,'\0',sizeof(completeMessage));
        ...
        strcat(buffer, "\n");
        //This is the encrypted message sent from server basck to client
        
        printf("%s", buffer);
     close(socketFD); // Close the socket
    }
    close(socketFD); // Close the socket not sure why i did this twice but I left it just to be sure, just really want to close this socket
 return 0;
 }
